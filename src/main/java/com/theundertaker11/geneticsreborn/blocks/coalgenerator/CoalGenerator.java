	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, @Nullable ItemStack heldItem, EnumFacing side, float hitX, float hitY, float hitZ) {
		...
		if(tEntity!=null&&tEntity instanceof GRTileEntityCoalGenerator&&hand==EnumHand.MAIN_HAND)
		{
			...
			else if(playerIn.isSneaking())
			{
				...
				playerIn.addChatMessage(new TextComponentString("Power Stored: "+tile.getEnergyStored(null)));
				playerIn.addChatMessage(new TextComponentString("Max RF/t: "+tile.getRfPerTick()));
				playerIn.addChatMessage(new TextComponentString("Ticks left on current fuel: "+tile.ticksleft));
			}
		}
		return true;
	}
